...
public class EmailExercise extends Exercise {
	...
	private String pedirPrioridad() {
		int intentos = 0;
		while (intentos < 3) {
			System.out.println("Ingrese la prioridad (Alto / Bajo):");
			String input = scanner.nextLine().trim();

			if (input.equalsIgnoreCase("Alto")) {
				return "Alto";
			} else if (input.equalsIgnoreCase("Bajo")) {
				return "Bajo";
			}

			System.out.println("Prioridad no válida. Las opciones son: Alto, Bajo.");
			intentos++;
		}
		System.out.println("Demasiados intentos inválidos. Volviendo al menú.");
		return null;
	...
	private void visualizarMails() {
		...
		System.out.println("\n¿Desea marcar el mail de mayor prioridad como resuelto? (si/no):");
		String respuesta = scanner.nextLine().trim().toLowerCase();

		if (respuesta.equals("si") || respuesta.equals("sí")) {
			Mail resuelto = temporal.dequeue();
			System.out.println("Mail resuelto: " + resuelto.toString());
		}
